...
function Companyname(props) {
  ...
  const getPlacePredictions = data => {
    if (data.input.length > 2) fetch(`https://api.leptonsoftware.com:9962/lepton/companyname?name=${data.input}`).then(res => res.json()).then(json => {
  ...
  }, [newAddresses]); // useEffect(() => { const styles = getComputedStyle(focusRef.current); let totalsize = Number(styles.width.slice(0, -2)) + Number(styles.paddingRight.slice(0, -2)) + Number(styles.paddingLeft.slice(0, -2)); getWidth(totalsize.toString() + "px"); }, []); const getWidth = (width) => { setWidth(width); }; useEffect(() => {
  //     if (props.searchval != '') {
  //         setValue(props.searchval);
  //         getPlacePredictions({ input: props.searchval });
  //         setShowHideList(true); setA(props.searchval);
  //         focusRef.current.focus();
  //     }
  // }, [props.searchval]);
  ...
  const handleKeyDown = e => {
    ...
    if (e.keyCode === 13) {
      ...
      setShowHideList(false); // setShowAutoComplete(false);
    }

    if (e.keyCode === 9) {
      // console.log("9",placePredictions[cursor].company_name)
      // setValue(placePredictions[cursor].company_name);
      setShowHideList(false); // props.parentCallback(placePredictions[cursor].company_name);
    ...
    // if (e.keyCode === 13) {
    //     setValue(placePredictions[cursor].description);
    //     getplace(placePredictions[cursor].place_id, props.apikey); setShowHideList(false);
    // }
    // if (e.keyCode === 9) { setShowHideList(false); } if (e.keyCode === 27) { setShowHideList(false); }
    // if (e.keyCode === 38 && cursor > 0) { if (e.keyCode === 13) { } setCursor(cursor - 1, () => { }); }
    // else if (e.keyCode === 40 && cursor < placePredictions.length - 1) { setCursor(cursor + 1); }
